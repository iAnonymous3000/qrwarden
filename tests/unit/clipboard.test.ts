...
describe("clipboard action lifetime", () => {
  it("rejects a trusted copy after live user activation expires", () => {
  ...
  it("copies only the escaped and bounded value that was displayed for review", () => {
    ...
    expect(writeText).toHaveBeenCalledExactlyOnceWith(reviewedField!.value);
  });

  it("does not copy invisible or truncated Wi-Fi password content beyond the display", () => {
  ...
  it("keeps the runtime busy until a trusted write settles", async () => {
    ...
    expect(writeText).toHaveBeenCalledExactlyOnceWith("[U+202E]display value");
    resolve();
    await write;
    await Promise.resolve();
