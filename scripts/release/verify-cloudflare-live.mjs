...
export async function verifyProbeResponse({ probe, response }) {
  ...
  for (const name of REPORTING_HEADER_NAMES) {
    if (response.headers.has(name)) {
      throw new Error(
        `${probe.pathname} returned reporting header ${name}; the generated Pages detach rule did not remove it — disable Network Error Logging for the serving zone or use a hostname where that setting is controllable; verification fails closed until live responses stop advertising reporting endpoints`,
      );
    }
  }
